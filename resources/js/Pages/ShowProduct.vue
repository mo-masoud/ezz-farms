...
<template>
	<AppLayout>
		<div class="relative w-full h-72">
			...
			>
				<h1 class="text-white text-3xl font-bold">FROMAGE A RACLETTE</h1>
				<h2 class="text-white text-lg">
					<Link :href="route('home')" class="mx-2">Home</Link> /
					<Link :href="route('products')" class="mx-2">Shop</Link> /
					<Link :href="route('products')" class="mx-2">Cheese</Link> /
					<span class="ml-2">Pecorino Romano</span>
				</h2>
			</div>
		</div>
			...
			<div class="flex space-x-12">
				<div class="w-1/2">
					<div class="relative border border-primary w-112 h-112 p-8">
						<img
							src="https://images.unsplash.com/photo-1602470520998-f4a52199a3d6?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80"
							alt="item"
							class="w-full h-full object-contain hover:scale-150 transition-all ease-in-out duration-200"
						/>
						<button @click="openImage = true" class="absolute top-5 right-5">
							<svg
				...
				<div class="w1/2">
					<h1 class="text-4xl font-bold">Pecorino Romano</h1>
					<span class="text-2xl font-bold mt-4">400 EG</span>
					...
					<div class="mt-20 text-xs">
						<div class="space-x-2">
							<span class="font-bold uppercase tracking-tighter"
								>Category :</span
							>
							<span class="text-gray-500">Cheese</span>
						</div>
						<div class="space-x-2">
							<span class="font-bold uppercase tracking-tighter">
								Product ID :
							</span>
							<span class="text-gray-500">141</span>
						</div>
					</div>
				</div>
			</div>
						...
						<TabPanel class="bg-gray-100 py-12 px-14">
							...
							>
								...
								<tr class="border-t border-gray-200">
									...
									<td class="text-sm italic font-semibold">
										{{ packaging.map((p) => p.text).toString() }}
									</td>
									<td v-for="i in 5" :key="i"></td>
								</tr>
							</table>
